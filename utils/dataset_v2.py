...
class FamilyDataset(Dataset):
    def __init__(self, root_dir, transform=None, filter_by_id=True, include_R=False, unique_per_family=True):
        self.root_dir = root_dir
        self.transform = transform
        self.data = []
        self.include_R = include_R  # R을 포함할지 여부를 결정하는 플래그 변수
        self.unique_per_family = unique_per_family  # 중복 제거 플래그 변수
        ...
        for family_folder in os.listdir(root_dir):
            ...
            if os.path.isdir(family_path):
                ...
                mother_images = sorted(os.listdir(mother_path))
                child_images = sorted(os.listdir(child_path))
                other_images = sorted(os.listdir(other_path))

                if self.unique_per_family:
                    # 중복 제거 플래그가 활성화된 경우, 각 폴더에서 첫 번째 이미지만 사용
                    if father_images and mother_images and child_images and other_images:
                        father_image_path = os.path.join(father_path, father_images[0])
                        mother_image_path = os.path.join(mother_path, mother_images[0])
                        child_image_path = os.path.join(child_path, child_images[0])
                        other_image_path = os.path.join(other_path, other_images[0])
                        ...
                        child_gender = 0 if "man" in child_images[0] else 1  # 0: man, 1: woman
                        other_gender = 0 if "man" in other_images[0] else 1  # 0: man, 1: woman
